...
use structure::{initial_employee_data,leave_report,Salary_report,final_report};
use self::calamine::{open_workbook, Error, Xlsx, Xls, Reader, RangeDeserializerBuilder,DataType};
use std::collections::HashMap;
use std::fs::File;
use std::io::{Write, BufReader, BufRead};

pub fn read_emp_data_file(app_name: &str) -> HashMap<String,initial_employee_data> {
...
pub fn read_dept_data_file(app_name: &str)->HashMap<String,String> {
    let config_params = get_configuration_parameters(app_name);
 ...
 pub fn read_salary_data_file(app_name: &str)->HashMap<String,Salary_report> {
    let config_params = get_configuration_parameters(app_name);
    ...
    while let Some(result) = iter.next() {
        ...
        let Salary = salary;
        let salary_statusReport = Salary_report { SalaryId, SalaryDate, Salary};
        mapofsalary.insert(EmpId, salary_statusReport);
    }
    return mapofsalary;
 }

 pub fn read_leave_data_file(app_name: &str)->HashMap<String,leave_report> {
    let config_params = get_configuration_parameters(app_name);
 ...
 pub fn get_empid_dept_id_pair(app_name: &str) ->HashMap<String, String> {
    ...
    tempmap = read_emp_data_file(app_name);
