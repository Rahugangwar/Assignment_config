...
use read_write_data_files::{read_dept_data_file,read_emp_data_file,read_leave_data_file,read_salary_data_file,get_empid_dept_id_pair};
use self::calamine::{open_workbook, Error, Xlsx, Xls, Reader, RangeDeserializerBuilder,DataType};
use structure::{initial_employee_data,Salary_report,leave_report,final_report};
...
pub fn get_final_report(app_name:&str) -> HashMap<String,final_report> {
    ...
    mapofemp = read_emp_data_file(app_name);
    mapofdept = read_dept_data_file(app_name);
    mapofsalary = read_salary_data_file(app_name);
    mapofleave = read_leave_data_file(app_name);
    ...
    for(key,value) in empid_deptid_pair.iter(){
        ...
        let mut salary_status = "".to_string();
        if empsalarydetail.Salary!=""{
            ...
            if check==true {
                salary_status = "Credited".to_string();
            }
            else{
                salary_status = "Not Credited".to_string();
            }
        }
        else {
            salary_status = "Not Credited".to_string();
        ...
        on_leave = naiveleaveto.signed_duration_since(naiveleavefrom).num_days().to_string();
        }
        else {
            on_leave = "0".to_string();
        }
        let employee = final_report{EmpId,EmpName,DeptTitle,MobileNo,Email,salary_status,on_leave};
        mapoffinalreport.insert(key.to_string(), employee);
    }
    return mapoffinalreport;
 }
 pub fn write_output(app_name:&str){
    let mut mapoffinalreport:HashMap<String, final_report >=HashMap::new();
    mapoffinalreport = get_final_report(app_name);
    let config_params= get_configuration_parameters(app_name);
    let path = config_params.final_output_file_path();
    let mut output = std::fs::File::create(path).expect("create failed");
    for (key,value) in mapoffinalreport.iter(){
        ...
        let salary_status = value.salary_status.to_owned();
        let on_leave = value.on_leave.to_owned();
        let delimiter = " - ".to_string();
        let v = vec![ID,name,depttitle,mobile,email,salary_status,on_leave];
        let s = v.connect(" - ");
