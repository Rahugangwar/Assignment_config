...
impl ConfigurationParameters {
    pub fn log_parameters(&self, logger: &Logger) {
        ...
        info!(logger, "final_output_file: {}", self.final_output_file_path());
    }
}
...
impl ConfigurationParameters {
    fn new_from_matches(matches: clap::ArgMatches) -> ConfigurationParameters {
        ...
        let leave_data_file_path = matches
            ...
            .to_string();
        let final_output_file_path = matches
            .value_of("final_output_file_path")
            .expect("Error getting `final_output_file_path`.")
            .to_string();
...
impl ConfigurationParameters {
    ...
    pub fn final_output_file_path(&self) -> &str {
        &self.final_output_file_path
    }
}
...
fn get_eligible_arguments_for_app(app_name: &str) -> clap::ArgMatches {
    App::new(app_name)
        ...
        .arg(
            Arg::with_name("final_output_file_path")
                .long("final-output-file")
                .value_name("Final Output File Path")
                .help("Path to write final output file.")
                .required(true)
        )
        .get_matches()
}
